// resources.bicep — Hub-and-spoke ACA private pattern, DEFAULT-DOMAIN variant.
//
...
//
// FOUR sample apps run in one INTERNAL ACA environment on a dedicated workload profile.
//
// NO CUSTOM DNS, NO CUSTOM CERT in this variant. The apps are reached via their
// auto-generated appN.<acaEnv.defaultDomain>.azurecontainerapps.io names, which the
// platform serves with a managed, publicly-trusted TLS certificate for free.
//
// Because the environment is INTERNAL, those names must resolve to the env's PRIVATE
// static IP. So a single Azure Private DNS zone for the default domain (wildcard
// A record) is linked to EVERY VNet (hub + ACA spoke + mgmt spoke), letting the
// Windows 11 VM resolve + reach the apps privately over HTTPS.
//
...
@description('Names of the sample apps. Each becomes a container app; reached via appN.<defaultDomain>.')
...
//   resolve the ACA default domain automatically. Both subnets
...
//   the apps from the VM. Egress forced through the firewall via UDR.
// ===========================================================================
...
// ===========================================================================
// NO custom DNS in this pattern: the apps use the environment's auto-generated
// *.<defaultDomain>.azurecontainerapps.io names, which come with a platform-
// managed, publicly-trusted TLS certificate for free. No Key Vault, no custom
// certificate, no custom DNS suffix needed.
// ===========================================================================
...
//   This is the ONLY DNS in this pattern. The apps are reached via their
//   auto-generated appN.<defaultDomain>.azurecontainerapps.io names, which the
//   platform serves with a managed, publicly-trusted TLS cert.
//   Because the environment is INTERNAL, those names must resolve to the env's
//   PRIVATE static IP — so we create a private zone for the default domain (with
//   a wildcard A record) and link it to every VNet (hub + ACA spoke + mgmt spoke),
//   so the Windows 11 VM (and anything in the mesh) resolves them privately.
//
...
output appFqdns array = [for (name, i) in appNames: apps[i].properties.configuration.ingress.fqdn]
output win11PrivateIp string = win11PrivateIp
output rdpConnect string = 'Connect RDP to ${dataPip.properties.ipAddress}:3389 (DNAT -> ${win11PrivateIp}:3389). From the VM, browse the four apps at their https://<app>.${acaEnv.properties.defaultDomain} URLs (see appFqdns output).'
output note string = 'Four internal apps run in an INTERNAL ACA env and are reached via their auto-generated *.${acaEnv.properties.defaultDomain} names (platform-managed TLS, no custom DNS/cert). A private DNS zone for the default domain is linked to hub + ACA + mgmt spokes so the Windows 11 VM resolves them to the env static IP ${acaEnv.properties.staticIp}. The VM has no public IP; RDP is published through the Azure Firewall DNAT on ${dataPip.properties.ipAddress}:3389.'
